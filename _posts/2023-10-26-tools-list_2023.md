...
`sudo apt install tmux fzf bat ripgrep zsh tldr jq ncdu asciinema duf fdupes hyperfine ctop bpytop speedtest-cli tre-command iproute2`

**Networking tools**
* nstat (part of iproute2) - checks network stats like tcp errors
  * `nstat -s` - display summary statistics
  * `nstat -a` - display all statistics
  * `nstat -z` - display statistics since last reset
  * `nstat -r` - display statistics in raw format
  * `nstat -h` - display help 
* `iptraf-ng` - (installed by iptraf-ng) real-time network statistics
  * `iptraf-ng -i eth0` - monitor eth0 interface
  * `iptraf-ng -i wlan0` - monitor wlan0 interface
  * `iptraf-ng -g` - display graphs
  * `iptraf-ng -s` - display statistics
  * `iptraf-ng -d` - display detailed info
  * `iptraf-ng -t` - run in text mode
  * `iptraf-ng -h` - display help
* `vnstat` - (installed by vnstat) network traffic monitor
* `iftop` - (installed by iftop) displays bandwidth usage on an interface
  * `iftop -i eth0` - monitor eth0 interface
  * `iftop -i wlan0` - monitor wlan0 interface
  * `iftop -B` - display bandwidth in bytes instead of bits
  * `iftop -n` - do not resolve hostnames
  * `iftop -p` - run in promiscuous mode
  * `iftop -t` - run in text mode
* `iperf` - (installed by iperf) network performance measurement tool
  * `iperf -s` - start iperf server
  * `iperf -c <server_ip>` - run iperf client to connect to server
  * `iperf -u` - use UDP instead of TCP
  * `iperf -t 60` - set test duration to 60 seconds
  * `iperf -p 5201` - set port to 5201
  * `iperf -R` - reverse test direction
  * `iperf -h` - display help
* `bmon` - (installed by bmon)bandwidth monitor and rate estimator
  * `bmon -p eth0` - monitor eth0 interface
  * `bmon -p wlan0` - monitor wlan0 interface
  * `bmon -r 1000` - set refresh interval to 1000ms
  * `bmon -o ascii:width=80` - set output to ascii with width of 80 characters
  * `bmon -o curses` - set output to curses
  * `bmon -o html` - set output to html
  * `bmon -o json` - set output to json
  * `bmon -o csv` - set output to csv
  * `bmon -o xml` - set output to xml
* `nload` - (installed by iftop) network traffic and bandwidth monitor
  * `nload eth0` - monitor eth0 interface
  * `nload wlan0` - monitor wlan0 interface
  * `nload -u M` - display in megabytes
  * `nload -u K` - display in kilobytes
  * `nload -t 1000` - set refresh interval to 1000ms
  * `nload -m` - show graphs in megabytes
* `ss` - utility to investigate sockets (comes with iproute2). Modern replacement for netstat.
  * `ss -tulnp` - list all listening sockets with process information
  * `watch ss -s` - real time socket stats
  * `ss -tuln | grep 443` - check if something is listening on port 443
  * `ss -tuln | grep LISTEN` - check all listening ports
  * `ss -tuln | grep ESTAB` - check all established connections
  * `ss -tuln | grep TIME_WAIT` - check all connections in TIME_WAIT state
* `ethtool eth0` - (installed by iproute2) get info and change settings of network interfaces
  * `ethtool -i eth0` - get driver info for eth0
  * `ethtool -S eth0` - get statistics for eth0
  * `ethtool -p eth0` - blink eth0 LED
  * `ethtool -s eth0 speed 1000 duplex full autoneg on` - set speed to 1000Mbps, full duplex, autonegotiation on
* `ip` - (comes with iproute2) show/manipulate routing, devices, policy routing and tunnels
  * `ip a` - show all IP addresses
  * `ip link` - show all network interfaces
* `mtr` - network diagnostic tool (installed by mtr)
  * `mtr -rw google.com` - run mtr in report mode with wide output to google.com
  * `mtr -c 10 google.com`
  * `mtr -c 10 google.com` - run mtr with 10 pings to google.com
  * `mtr -i 0.5 google.com` - run mtr with 0.5 second interval to google.com
  * `mtr -p google.com` - run mtr in plain text mode to google.com
  * `mtr -b google.com` - run mtr with both hostnames and IP addresses to google.com
  * `mtr -h google.com` - display help for mtr
  * `mtr -h` - display help for mtr 
...
- [These actually worked for apt](#these-actually-worked-for-apt)
    - [Things I do use](#things-i-do-use)
      - [Replacement Commands](#replacement-commands)
    - [Things I want to try](#things-i-want-to-try)
      - [PM, Data Management and Notes Tools](#pm-data-management-and-notes-tools)
      - [Better CLI Tools](#better-cli-tools)
      - [1. tmux – Terminal Multiplexer](#1-tmux--terminal-multiplexer)
      - [3. fzf – Fuzzy Finder](#3-fzf--fuzzy-finder)
      - [4. bat – Enhanced Cat Command](#4-bat--enhanced-cat-command)
      - [5. eza – Modern Alternative to ls](#5-eza--modern-alternative-to-ls)
      - [6. ripgrep (rg) – Fast File Search](#6-ripgrep-rg--fast-file-search)
      - [7. Zsh – Z Shell](#7-zsh--z-shell)
      - [10. tldr – Simplified Man Pages](#10-tldr--simplified-man-pages)
      - [11. jq – JSON Processor](#11-jq--json-processor)
      - [12. ncdu – Disk Usage Analyzer](#12-ncdu--disk-usage-analyzer)
      - [13. asciinema – Terminal Session Recorder](#13-asciinema--terminal-session-recorder)
      - [CLI tools you won't be able to live without 🔧 - DEV Community](#cli-tools-you-wont-be-able-to-live-without----dev-community)
  - [Utils](#utils)
    ...
    - [`sd` - Find and replace _(better `sed`)_](#sd---find-and-replace-better-sed)
...
### [](#-raw-exa-endraw-listing-files-better-raw-ls-endraw-)[`exa`](https://github.com/ogham/exa) - Listing Files _(better `ls`)_
...
### [](#-raw-xsel-endraw-access-the-clipboard)[`xsel`](https://github.com/kfish/xsel) - Access the clipboard
...
### [](#-raw-browsh-endraw-cli-web-browser)[`browsh`](https://github.com/browsh-org/browsh) - CLI web browser
...
[![browsh-example-usage](https://res.cloudinary.com/practicaldev/image/fetch/s--sO3TkLzU--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://i.ibb.co/S7nLFX5/browsh.gif)](https://res.cloudinary.com/practicaldev/image/fetch/s--sO3TkLzU--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://i.ibb.co/S7nLFX5/browsh.gif)
...
### [](#-raw-buku-endraw-bookmark-manager)[`buku`](https://github.com/jarun/buku) - Bookmark manager
...
### [](#-raw-cmus-endraw-music-browser-player)[`cmus`](https://github.com/cmus/cmus) - Music browser / player
...
### [](#-raw-cointop-endraw-track-crypto-prices)[`cointop`](https://github.com/cointop-sh/cointop) - Track crypto prices
...
### [](#-raw-ddgr-endraw-search-the-web-from-the-terminal)[`ddgr`](https://github.com/jarun/ddgr) - Search the web from the terminal
...
### [](#-raw-micro-endraw-code-editor-better-raw-nano-endraw-)[`micro`](https://github.com/zyedidia/micro) - Code editor _(better `nano`)_
...
### [](#-raw-khal-endraw-calendar-client)[`khal`](https://github.com/pimutils/khal) - Calendar client
...
### [](#-raw-mutt-endraw-email-client)[`mutt`](https://gitlab.com/muttmua/mutt) - Email client
...
### [](#-raw-newsboat-endraw-rss-atom-news-reader)[`newsboat`](https://github.com/newsboat/newsboat) - RSS / ATOM news reader
...
### [](#-raw-rclone-endraw-manage-cloud-storage)[`rclone`](https://github.com/rclone/rclone) - Manage cloud storage
...
### [](#-raw-taskwarrior-endraw-todo-task-management)[`taskwarrior`](https://github.com/GothenburgBitFactory/taskwarrior) - Todo + task management
...
### [](#-raw-tuir-endraw-terminal-ui-for-reddit)[`tuir`](https://gitlab.com/ajak/tuir) - Terminal UI for Reddit
...
### [](#-raw-httpie-endraw-http-api-testing-testing-client)[`httpie`](https://github.com/httpie/httpie) - HTTP / API testing testing client
...
### [](#-raw-lazydocker-endraw-full-docker-management-app)[`lazydocker`](https://github.com/jesseduffield/lazydocker) - Full Docker management app
...
### [](#-raw-lazygit-endraw-full-git-management-app)[`lazygit`](https://github.com/jesseduffield/lazygit) - Full Git management app
...
### [](#-raw-kdash-endraw-kubernetes-dashboard-app)[`kdash`](https://github.com/kdash-rs/kdash/) - Kubernetes dashboard app
...
### [](#-raw-gdpdashboard-endraw-visual-gdp-debugger)[`gdp-dashboard`](https://github.com/cyrus-and/gdb-dashboard) - Visual GDP debugger
...
### [](#-raw-ngrok-endraw-reverse-proxy-for-sharing-localhost)[`ngrok`](https://ngrok.com/) - Reverse proxy for sharing localhost
...
### [](#-raw-tmate-endraw-share-a-terminal-session-via-internet)[`tmate`](https://tmate.io/) - Share a terminal session via internet
...
### [](#-raw-asciinema-endraw-recording-sharing-terminal-sessions)[`asciinema`](https://asciinema.org/) - Recording + sharing terminal sessions
...
### [](#-raw-navi-endraw-interactive-cheat-sheet)[`navi`](https://github.com/denisidoro/navi) - Interactive cheat sheet
...
### [](#-raw-transfersh-endraw-fast-file-sharing)[`transfer.sh`](https://github.com/dutchcoders/transfer.sh/) - Fast file sharing
...
### [](#-raw-surge-endraw-deploy-a-site-in-seconds)[`surge`](https://surge.sh/) - Deploy a site in seconds
...
### [](#-raw-wttrin-endraw-check-the-weather)[`wttr.in`](https://github.com/chubin/wttr.in) - Check the weather
...
### [](#-raw-cowsay-endraw-have-an-ascii-cow-say-your-message)[`cowsay`](https://en.wikipedia.org/wiki/Cowsay) - Have an ASCII cow say your message
...
### [](#-raw-figlet-endraw-output-text-as-big-ascii-art-text)[`figlet`](http://www.figlet.org/) - Output text as big ASCII art text
...
### [](#-raw-lolcat-endraw-make-console-output-raibow-colored)[`lolcat`](https://github.com/busyloop/lolcat) - Make console output raibow colored
...
### [](#-raw-neofetch-endraw-show-system-data-and-ditstro-info)[`neofetch`](https://github.com/dylanaraps/neofetch) - Show system data and ditstro info
...
#### [](#what-wasnt-included)What wasn't included
...
_Very, very quick script to quickly generate repository badges_  
...
> _**[Lissy93/repo-badge-maker](https://github.com/Lissy93/repo-badge-maker)** is licensed under [MIT](https://gist.github.com/Lissy93/143d2ee01ccc5c052a17) © [Alicia Sykes](https://aliciasykes.com/) 2022._  
> For information, see [TLDR Legal > MIT](https://tldrlegal.com/license/mit-license)
